...
- `index.html` — clasificación pública (la que ven todos), con calendario completo del Tour
- `admin.html` — panel donde metes los resultados cada día (solo tú)
- `data.json` — participantes, resultados, calendario de las 21 etapas (se actualiza solo, vía commits)
...
- `estilo.css` — estilos, imagen oficial Tour de Francia
- `assets/` — logo y favicon del Tour (súbela también, es una **carpeta**, no un archivo suelto)
...
2. Sube todos estos archivos a la raíz del repo, **incluida la carpeta `assets/` completa** (arrástrala entera al subir, GitHub respeta la subcarpeta).
...
**Truco:** arriba del formulario hay un desplegable con las 21 etapas del
calendario oficial 2026 — al elegir una, se rellenan solos el número y el
nombre (salida → meta).

**⚠️ En la última etapa (la 21, París):** deja en blanco General, Puntos,
Montaña y Equipos — solo mete el Top 10 de "etapa". Esos cuatro ya no se
puntúan como "líder del día" el último día; en su lugar van en "Bonus final"
(ver paso 6).

**Nota sobre corredores que no están en tu porra:** es normal que el
resultado real del Tour incluya corredores que nadie eligió — el panel te
avisará (aviso amarillo) pero te dejará guardar igualmente, simplemente esa
posición no dará puntos a nadie.
